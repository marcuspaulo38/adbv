...
const ViewTable = ({ invoiceList, columns = viewColumns }) => {
  return (
    <Table columns={columns} dataSource={invoiceList} pagination={false} />
  );
};
    ...
    },
    {
      ...
      render: (text, singleInvoice) => (
        <Input
          ...
          onChange={event => {
            editableInvoice.invoiceList[singleInvoice.key - 1].qty = 'chablau';
